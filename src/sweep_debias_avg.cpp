/**
 * sweep_debias_avg.cpp
 *
 * Sweep debiased averaging distributed logistic regression across a range of
...
int main(int argc, char** argv)
{
  ...
  DebiasAvg davg(lambda, partitions, ratio);
  davg.numThreads = totalThreads;
  davg.verbose = verbose;
  davg.seed = seed;
  // The first run needs to be done separately to populate the LIBLINEAR
  // features members.
  davg.Train(trainData, trainLabels);
  ...
  for (size_t i = start; i < count; ++i)
  {
    ...
    DebiasAvg davgThread(davg);

    arma::wall_clock cThread;
    cThread.tic();
    davgThread.lambda = lambdaThread;
    davgThread.numThreads = totalThreads;
    davgThread.Retrain(); // same dataset
    const double trainTimeThread = cThread.toc();

    const std::tuple<double, double> accsThread = TrainTestAccuracy(
        davgThread, trainData, trainLabels, testData, testLabels);

    f << "debias_avg-" << partitions << "," << i << "," << lambdaThread << ","
        << powThread << "," << davgThread.modelNonzeros << ","
        << std::get<0>(accsThread) << "," << std::get<1>(accsThread) 
        << "," << trainTimeThread << endl;
    cout << "Debiased averaging, " << partitions << " partitions, lambda 10^"
        << powThread << ": " << trainTimeThread << "s training time; "
        << davgThread.modelNonzeros << " nonzeros; "
