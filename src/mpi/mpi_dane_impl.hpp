 ...
 */
#ifndef MPI_DANE_IMPL_HPP
#define MPI_DANE_IMPL_HPP
...
inline MPIDANE::MPIDANE(const std::string& trainBaseFilename,
...
inline void MPIDANE::TrainNaive()
{
...
inline void MPIDANE::Update(double alpha)
{
  ...
  liblinear::lr_gradient((const liblinear::feature_node**) liblinearTrainData[0],
                         ...
                         active_dims,
                         active_size);
  arma::rowvec igrad = grad / trainData.n_cols;     // normalized version
  ...
  // Now compute the global gradient on main worker
  c.tic();
  arma::rowvec ugrad;
  if (worldRank == 0)
  {
    // Compute normalized global gradient on the main node
    ugrad = arma::sum(grads / (double) totalPoints, 1).t();
  }
  else
  {
    // on other nodes, initialize the correct size to receive gradient.
    ugrad.set_size(model.n_elem);
  }
  const double gradComputeTime = c.toc();

  // Send global gradient from main worker to all other workers.
  ...
  c.tic();
  for (size_t i = 0; i < ugrad.n_elem; i += 128)
  {
    const size_t len = (i + 128 <= ugrad.n_elem) ? 128 : (ugrad.n_elem - i);
    MPI_Bcast(ugrad.memptr() + i,
              len,
              MPI_DOUBLE,
              0,
              MPI_COMM_WORLD);
  }
  const double distributeGradTime = c.toc();

  // Solve shifted DANE/CSL optimization on every node
  c.tic();
  ...
  // Collect all updated models on the main node.  `models` is only used on the main
  // node.
  c.tic();
  arma::mat models;
  if (worldRank == 0)
  ...
  MPI_Gather(model.memptr(), model.n_elem, MPI_DOUBLE,
      models.memptr() /* models.col(i) will be model i */, model.n_elem,
      MPI_DOUBLE, 0 /* gather on the main node */, MPI_COMM_WORLD);
  const double modelCollectionTime2 = c.toc(); 
  ...
  if (worldRank == 0)
  {
    ...
    c.tic();
    model = arma::mean(models, 1).t();
    const double modelAverageTime = c.toc();

    std::cout << "DANE model timing for lambda " << lambda << ":\n";
    std::cout << " - Broadcast model time: " << distributeModelTime << "s.\n";
    std::cout << " - Gradient collection time: " << gradCollectionTime << "s.\n";
    std::cout << " - Global gradient compute time: " << gradComputeTime << "s.\n";
    std::cout << " - Broadcast gradient time: " << distributeGradTime << "s.\n";
    std::cout << " - CSL solve time: " << CSLTime << "s.\n";
    std::cout << " - Collect all models time: " << modelCollectionTime2 << "s.\n";
    std::cout << " - Average models time: " << modelAverageTime << "s.\n";
  }
}

inline double MPIDANE::TrainAccuracy()
{
...
inline double MPIDANE::TestAccuracy()
{
  // Compute accuracy on local system.
...
inline size_t MPIDANE::CountCorrect(const arma::sp_mat& data,
...
inline void MPIDANE::DistributeModel()
{
  // Send model from main worker to all other workers.
  // It seems like MVAPICH2 segfaults when we try to send the entire buffer at
  // once, so we break it up into messages of 128 elements (1 kb).
