 ...
 * sweep_acowa.cpp
 *
 * Sweep ACOWA distributed logistic regression across a range of lambda
 * values, storing the train/test accuracies in a CSV file.
 */
#include "acowa.hpp"
#include "libsvm.hpp"
...
void help(char** argv)
{
  cout << "Usage: " << argv[0] << " input_data.svm output_file.csv seed "
       << "min_reg max_reg count partitions [start [verbose]]"
       << endl
       << endl
...
int main(int argc, char** argv)
{
  ...
  if (start == 0)
  {
    f << "method,index,lambda,lambda_pow,nnz,train_acc,test_acc" << endl;
  }
  ...
  ACOWA<arma::sp_mat> owa(lambda, partitions);
  ...
  for (size_t i = start; i < count; ++i)
  {
    ...
    ACOWA<arma::sp_mat> owaThread(owa);
    ...
    #pragma omp critical
    {
      f << "acowa-" << partitions << "," << i << "," << lambdaThread
          << "," << powThread << "," << owaThread.modelNonzeros << ","
          << std::get<0>(accsThread) << "," << std::get<1>(accsThread) << endl;
      cout << "ACOWA, " << partitions << " partitions, lambda "
          << "10^" << powThread << ": " << trainTimeThread << "s training time;"
