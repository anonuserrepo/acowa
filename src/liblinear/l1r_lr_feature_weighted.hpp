#ifndef LIBLINEAR_L1R_LR_FEATURE_WEIGHTED_HPP
#define LIBLINEAR_L1R_LR_FEATURE_WEIGHTED_HPP
...
//   * Feature weight support is added.  (Pass as prob_col->W.)
//   * The maximum number of inner iterations is configurable.
//   * Use local RNG to avoid glibc rand() global lock..

inline int solve_l1r_lr_feature_weighted(const problem *prob_col, const parameter *param, double *w, double Cp, double Cn, double eps)
{
  int l = prob_col->l;
  ...
  for(j=0; j<w_size; j++)
  {
    ...
    x = prob_col->x[j];
    nnz_per_feature[j] = 0;
    double Wj = prob_col->W[j];
    while(x->index != -1)
    {
      int ind = x->index-1;
      double val = x->value * Wj;
      exp_wTx[ind] += w[j]*val;
      if(y[ind] == -1)
  ...
  while(newton_iter < max_newton_iter)
  {
    ...
    for(s=0; s<active_size; s++)
    {
      ...
      double tmp = 0;
      x = prob_col->x[j];
      double Wj = prob_col->W[j]; // feature weight for dimension j
      while(x->index != -1)
      {
        int ind = x->index-1;
        Hdiag[j] += x->value*x->value*Wj*Wj*D[ind];
        tmp += x->value*Wj*tau[ind];
        x++;
      }
    ...
    while(iter < max_iter)
    {
      ...
      for(s=0; s<QP_active_size; s++)
      {
        ...
        x = prob_col->x[j];
        Wj = prob_col->W[j];
        sparse_operator::waxpy(z, x, Wj, xTd);
      }
    ...
    if(num_linesearch >= max_num_linesearch)
    {
      ...
      for(int i=0; i<w_size; i++)
      {
        if(w[i]==0) continue;
        x = prob_col->x[i];
        double Wi = prob_col->W[i];
        sparse_operator::waxpy(w[i], x, Wi, exp_wTx);
      }
