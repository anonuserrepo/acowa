#ifndef LIBLINEAR_L1R_LR_WEIGHTED_HPP
#define LIBLINEAR_L1R_LR_WEIGHTED_HPP
...
//   * Instance weights are supported (via GETI(i) --> (i) transformation)
//   * The maximum number of inner iterations is configurable.
//   * Use local RNG to avoid glibc rand() global lock..

inline int solve_l1r_lr_weighted(const problem *prob_col, const parameter *param, double *w, double Cp, double Cn, double eps)
{
  int l = prob_col->l;
  ...
  for(j=0; j<l; j++)
    if(y[j] == 1)
      v += C[j]*log(1+1/exp_wTx[j]);
    else
      v += C[j]*log(1+exp_wTx[j]);
