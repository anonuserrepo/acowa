...
int main(int argc, char** argv)
{
  ...
  MPIDebiasAvg navg(inputFilePrefix, testFilePrefix, extension, dataDim, lambda,
      verbose);
  for (size_t i = start + 1; i < count; ++i)
  {
    ...
    if (worker == 0)
    {
      f << "debias_avg-" << worldSize << "," << i << "," << lambda << ","
          << pow << "," << nonzeros << ","
          << trainAcc << "," << testAcc << "," << trainTime << endl;
      cout << "Debiased averaging, " << worldSize << " partitions, lambda 10^"
