 ...
 * sweep_mpi_acowa.cpp
 *
 * Sweep MPI ACOWA logistic regression across a range of lambda values, storing
 * the train/test accuracies in a CSV file.
 */
#include <mpi.h>
#include "mpi_acowa.hpp"
#include "../libsvm.hpp"
...
int main(int argc, char** argv)
{
  ...
  MPIACOWA owa(inputFilePrefix, testFilePrefix, extension, dataDim, lambda,
      verbose);
  for (size_t i = start + 1; i < count; ++i)
  {
    ...
    if (worker == 0)
    {
      f << "acowa-" << worldSize << "," << i << "," << lambda << ","
          << pow << "," << nonzeros << ","
          << trainAcc << "," << testAcc << "," << trainTime << endl;
      cout << "ACOWA, " << worldSize << " partitions, lambda 10^"
